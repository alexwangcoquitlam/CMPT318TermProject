...
#Global_intensity
#Global_active_power
ma <- rollmean(dfTime$Global_active_power, n)
...
ma3 <- rollmean(dfTime$Global_active_power, m)
...
#Global_intensity
anomplotdata <- data.frame(dfTime$Global_active_power, Dates, dfTime$Time, ma2, ma4)


#plot line and moving average on plot
plot1 <- ggplot(anomplotdata, aes(Dates, dfTime.Global_active_power))  + geom_line(size = 0.1, alpha =.5) + 
